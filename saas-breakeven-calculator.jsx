...
function Dot({ on, onClick }) {
  ...
  return (
    <span onClick={onClick} title={on ? "點擊停用（計算時以中性值代入）" : "點擊啟用"}
      style={{ cursor: "pointer", color: on ? "#34d399" : "#5a6a7e", fontSize: 11, userSelect: "none" }}>
      {on ? "●" : "○"}
    </span>
  );
}
...
function NumInput({ label, value, onChange, prefix, suffix, width = 100, small, tip, on, onToggle }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 3, opacity: off ? 0.45 : 1 }}>
      <div style={{ fontSize: 11, color: "#7a8a9e", display: "flex", alignItems: "center", gap: 5 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        {prefix && <span style={{ fontSize: 12, color: "#7a8a9e" }}>{prefix}</span>}
        <input type="text" value={value} onChange={e => onChange(e.target.value)} disabled={off}
          style={{
            ...
            fontSize: small ? 16 : 20, fontWeight: 700, color: "#e8edf3",
            width, outline: "none", transition: "border-color 0.2s",
          }}
          onFocus={e => e.target.style.borderColor = "#f59e0b"}
          onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.08)"} />
        {suffix && <span style={{ fontSize: 12, color: "#7a8a9e" }}>{suffix}</span>}
      </div>
      {tip && <div style={{ fontSize: 10, color: "#4a5568" }}>{tip}</div>}
    </div>
  );
}
...
export default function AIPricingCalc() {
  ...
  return (
    <div style={{
      ...
      background: "linear-gradient(160deg, #0c0f14 0%, #131820 40%, #0e1117 100%)",
      color: "#d0d8e4", minHeight: "100vh", padding: "28px 16px",
    }}>
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, margin: "8px 0 0", color: "#e8edf3", lineHeight: 1.3 }}>
            完整損益模擬（含流失 · 擴容 · 續購）
          </h1>
          <p style={{ color: "#5a6a7e", fontSize: 12, margin: "6px 0 0" }}>
        ...
        <div style={{ ...card, marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: "#e8edf3" }}>① 每輪對話 API 成本</span>
          </div>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 11, color: "#7a8a9e" }}>單輪總成本</span>
        ...
        <div style={{ ...card, marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 15 }}>📦</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: "#e8edf3" }}>② 對話包方案設計</span>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 12 }}>
            {packs.map((pack, i) => {
              ...
              return (
                <div key={i} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: 12, border: "1px solid rgba(255,255,255,0.06)", opacity: packOn ? 1 : 0.45 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                    ...
                    <input value={pack.name} onChange={e => updatePack(i, "name", e.target.value)} disabled={!packOn}
                      style={{ background: "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,0.1)", color: "#e8edf3", fontSize: 13, fontWeight: 700, flex: 1, minWidth: 0, padding: "2px 0 4px", outline: "none", textDecoration: packOn ? "none" : "line-through" }} />
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {[
                      ...
                      <div key={r.f} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <span style={{ fontSize: 11, color: "#7a8a9e" }}>{r.l}</span>
                        <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                          {r.pre && <span style={{ fontSize: 10, color: "#7a8a9e" }}>{r.pre}</span>}
                          <input value={r.v} onChange={e => updatePack(i, r.f, e.target.value)}
                            style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, padding: "3px 6px", fontSize: 13, fontWeight: 600, color: "#e8edf3", width: 65, textAlign: "right", outline: "none" }} />
                        </div>
                      </div>
                    ))}
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 11, color: "#7a8a9e" }}>用戶佔比</span>
                      <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                        <input value={packMix[i]} onChange={e => updateMix(i, e.target.value)}
                          style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, padding: "3px 6px", fontSize: 13, fontWeight: 600, color: "#e8edf3", width: 35, textAlign: "right", outline: "none" }} />
                        <span style={{ fontSize: 10, color: "#7a8a9e" }}>%</span>
                      </div>
                    </div>
                  </div>
                  <div style={{ marginTop: 8, padding: "6px 0 0", borderTop: "1px solid rgba(255,255,255,0.06)", fontSize: 11 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                      <span style={{ color: "#7a8a9e" }}>API 成本</span><span style={{ color: "#ef4444" }}>{fmt(apiCost)}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ color: "#7a8a9e" }}>毛利</span>
        ...
        <div style={{ ...card, marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 15 }}>⭐</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: "#e8edf3" }}>② 點數包方案設計</span>
            <span style={{ fontSize: 10, color: "#5a6a7e", marginLeft: "auto" }}>
            ...
            {creditPacks.map((pack, i) => {
              ...
              return (
                <div key={i} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: 12, border: "1px solid rgba(255,255,255,0.06)", opacity: packOn ? 1 : 0.45 }}>
                  ...
                  <div style={{ marginTop: 8, padding: "6px 0 0", borderTop: "1px solid rgba(255,255,255,0.06)", fontSize: 11 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                      <span style={{ color: "#7a8a9e" }}>單點售價</span><span style={{ color: "#e8edf3" }}>NT${unitPrice.toFixed(3)}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ color: "#7a8a9e" }}>單點毛利</span>
          ...
          <div style={{ fontSize: 10, color: "#5a6a7e" }}>
        ...
        <div style={{ ...card, marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: "#e8edf3" }}>③ 營運基礎參數</span>
          </div>
        ...
        <div style={{ ...card, marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 15 }}>🔄</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: "#e8edf3" }}>
              ④ {activeTab === "credit" ? "用戶流失與點數消耗" : "用戶流失與續購行為"}
            </span>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
            <div style={{ opacity: enabled.churnRate ? 1 : 0.45 }}>
              <div style={{ fontSize: 11, color: "#7a8a9e", marginBottom: 4, display: "flex", alignItems: "center", gap: 5 }}>
          ...
          {activeTab === "credit" && (
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 14, alignItems: "center" }}>
                ...
                <div style={{ fontSize: 11, color: "#7a8a9e" }}>
        ...
        <div style={{ ...card, marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 15 }}>🖥️</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: "#e8edf3" }}>⑤ GPU 伺服器階梯擴容</span>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <div style={{ fontSize: 11, color: "#7a8a9e", marginBottom: 4 }}>目前模擬需要</div>
        ...
        <div style={{ ...card, marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 15 }}>📈</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: "#e8edf3" }}>⑥ 用戶成長率</span>
          ...
          <div style={{
            ...
            background: enabled.monthlyGrowth ? "rgba(52,211,153,0.06)" : "rgba(255,255,255,0.02)",
            border: `1px solid ${enabled.monthlyGrowth ? "rgba(52,211,153,0.25)" : "rgba(255,255,255,0.06)"}`,
          }}>
            <Dot on={enabled.monthlyGrowth} onClick={() => toggle("monthlyGrowth")} />
            <span onClick={() => toggle("monthlyGrowth")}
              style={{ fontSize: 12, color: "#7a8a9e", cursor: "pointer", textDecoration: enabled.monthlyGrowth ? "none" : "line-through" }}>
              每月統一用戶成長率
            </span>
            <span style={{ fontSize: 22, fontWeight: 700, color: enabled.monthlyGrowth ? "#34d399" : "#5a6a7e", minWidth: 60 }}>
            ...
            <span style={{ fontSize: 10, color: "#4a5568", width: "100%" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, opacity: enabled.monthlyGrowth ? 0.4 : 1 }}>
            {["Q1（1-3月）", "Q2（4-6月）", "Q3（7-9月）", "Q4（10-12月）"].map((label, qi) => {
              ...
              return (
                <div key={qi} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: 12, border: `1px solid ${colors[qi]}22`, opacity: qOn ? 1 : 0.45 }}>
                  <div style={{ fontSize: 11, color: "#7a8a9e", marginBottom: 2, display: "flex", alignItems: "center", gap: 5 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8, marginBottom: 12 }}>
          {[
            ...
            <div key={i} style={{ ...card, textAlign: "center", padding: "12px 8px" }}>
              <div style={{ fontSize: 9, color: "#5a6a7e", marginBottom: 5 }}>{m.label}</div>
        ...
        {(() => {
          ...
          return (
            <div style={{
              ...
              <span style={{ fontSize: 12, color: "#5a6a7e" }}>前 3 月累計：{fmt(m3?.cumProfit || 0)}</span>
            </div>
          );
        })()}
        ...
        <div style={{ ...card, padding: 18, marginBottom: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 14, color: "#e8edf3" }}>12 個月累計損益趨勢</div>
          <div style={{ display: "flex", gap: 12, marginBottom: 12, fontSize: 10, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ color: "#7a8a9e" }}>營收（依季度著色）</span>
            {["Q1", "Q2", "Q3", "Q4"].map((q, i) => {
            ...
            {data.months.map(m => {
              ...
              return (
                <div key={m.month} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  <div style={{ fontSize: 8, color: "#5a6a7e", marginTop: 3 }}>{m.month}月</div>
                </div>
              );
            })}
          </div>
        ...
        {/* Monthly table */}
        <div style={{ ...card, padding: 18, marginBottom: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12, color: "#e8edf3" }}>逐月明細</div>
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                  {["月份", "季/成長", "活躍用戶", "新增", activeTab === "credit" ? "等效點包數" : "購買次數", "GPU台數", "營收", "API成本", "固定+GPU", "月損益", "累計損益"].map(h => (
                    <th key={h} style={{ padding: "7px 5px", textAlign: "right", color: "#5a6a7e", fontWeight: 500, fontSize: 9 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.months.map(m => {
                  const qc = ["#f59e0b", "#3b82f6", "#a78bfa", "#34d399"][m.quarter - 1];
                  return (
                    <tr key={m.month} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", background: m.month <= 3 ? "rgba(245,158,11,0.03)" : "transparent" }}>
                      <td style={{ padding: "6px 5px", textAlign: "right", color: "#e8edf3", fontWeight: m.month <= 3 ? 600 : 400, fontSize: 10 }}>
                      ...
                      <td style={{ padding: "6px 5px", textAlign: "right", color: "#7a8a9e" }}>+{m.newUsers.toLocaleString()}</td>
                      ...
                      <td style={{ padding: "6px 5px", textAlign: "right", color: "#e8edf3" }}>{fmt(m.monthRevenue)}</td>
          ...
          <div style={{ fontSize: 9, color: "#5a6a7e", marginTop: 6 }}>
            ⚡ = 目標回本期 ｜
            {activeTab === "credit"
        ...
        <div style={{
          ...
          fontSize: 11, color: "#7a8a9e", lineHeight: 1.8,
        }}>
